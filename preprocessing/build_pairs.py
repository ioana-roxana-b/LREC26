...
def build_pairs_from_merged(merged: pd.DataFrame) -> pd.DataFrame:
    ...
    Skips any rows where A or B are empty or invalid.
    """
    pairs = []
    for row_index in range(len(merged) - 1):
        speaker_a = str(merged.at[row_index, "speaker"]).strip()
        speaker_b = str(merged.at[row_index + 1, "speaker"]).strip()

        if not speaker_a or not speaker_b:
            continue
        if speaker_a.lower() in {"none", "nan", "na"} or speaker_b.lower() in {"none", "nan", "na"}:
            continue

        addressee_set = {
            str(x).strip()
            for x in (merged.at[row_index, "addressees"] or [])
            if str(x).strip()
        }

        if speaker_b in addressee_set and speaker_a != speaker_b:
            pairs.append(
                {
                    "A": speaker_a,
                    "B": speaker_b,
...
def build_pairs_from_merged_nonadjacent(merged: pd.DataFrame) -> pd.DataFrame:
    ...
    for row_index in range(len(merged) - 2):
        speaker_a = str(merged.at[row_index, "speaker"]).strip()
        speaker_b = str(merged.at[row_index + 2, "speaker"]).strip()
...
def build_pairs_randomized_from_adjacent(pairs_adj: pd.DataFrame, seed: int = 123 ) -> pd.DataFrame:
    ...
    if pairs_adj is None or pairs_adj.empty:
        return pairs_adj.copy()

    work = pairs_adj.copy()
    work["A"] = work["A"].astype(str).str.strip()
    work["B"] = work["B"].astype(str).str.strip()

    bad = {"", "none", "nan", "na"}
    mask_valid = (~work["A"].str.lower().isin(bad)) & (~work["B"].str.lower().isin(bad))
    work = work.loc[mask_valid].reset_index(drop=True)
    if work.empty:
        return work
